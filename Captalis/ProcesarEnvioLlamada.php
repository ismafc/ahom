...
<?php
    ...
    include("./Lib/captalisLlamada.inc");
...
$n = procesarFactura($content, $idMiembro, $edad_titular, $idParentesco, $descripcion, $idFactura);
    ...
    $costes = obtenerCostesCaptalis($facturas, $numero_movil_llamante);

    // ----------------
    // Servicios Ahorro
    // ----------------

    $idContratos = obtenerContratosCaptalis();
    foreach($idContratos as $idContrato) {
            ...
            if (empty($idsCompatibilidades) == false) {
                    ...
                    foreach ($facturas as $factura) {
                            foreach ($idsCompatibilidades as $idCompatibilidad) {
                                    AplicarServiciosAhorroALlamadasSeleccionadasCaptalis($factura, $idContrato, $idCompatibilidad, false, $numero_movil_llamante);
                            }
                    }
            }	
    }
...
<table cellspacing="0" cellpadding="0" border="0" width="100%">
    ...
    <?php
    crearTablaResultadosBasicosLlamadaCAPTALIS($idMiembro, $numero_movil_llamante);
    ?>
    <tr><td colspan="3" style="height:30px"></td></tr>
    <tr>
        <td class="texto" colspan="3" align="center"><a href="http://www.captalis.com/telefonos-moviles/ahorrar-llamadas/" target="_top">Volver al formulario inicial</a></td>
    </tr>
</table>
<?php
if (esProvisional ($login, $password)){
