...
<?php
	...
	if (isset ($_GET["id"])){
		...
		$id = $_GET["id"];
		$modificar = 1;
		echo "<br /><strong>MODIFICAR TARJETA</strong><br /><br />";
	}else{
		// añadir;
		echo "<br /><strong>NUEVA TARJETA</strong><br /><br />";
	}
	...
	if (isset ($_POST['enviar'])){
		...
		if ($modificar == 1){	
			...
			$sql = "INSERT INTO tarjetas (nombre, idOperador, urlTarifa, urlTarifaOperador) ".
				"VALUES ('$nombre', '$idOperador', '$urlTarifa')";
		}
		...
		if ($result==0)
			...
			echo "<br>Tarjeta añadida correctamente<br>";
		...
		if ($modificar == 1){
			$sql = "SELECT nombre, idOperador, urlTarifa, urlTarifaOperador FROM tarjetas WHERE id=$id";
...
?>
    <form action="GestionTarjetasProcesar.php<?php if ($modificar == 1) echo "?id=".$id;?>" method="post">
